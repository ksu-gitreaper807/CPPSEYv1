...
// ============================================================================
// OPT: Precomputed valid-combo index table
//
// All three post-processing loops over the 64 combo slots skip the 8 diagonal
// entries where (i/8)==(i%8) (same min and max frame → no transition).
// Rather than re-checking every iteration, we build the 56 valid indices once
// at compile time and iterate only those.  This removes a data-dependent branch
// from every combo-scan loop and allows the compiler to fully unroll or
// vectorise the reduction.
// ============================================================================
static constexpr int kNumValidCombos = 56;

// Build at compile time: all i in [0,64) where (i/8) != (i%8).
static constexpr std::array<int, kNumValidCombos> kValidComboIndices = []() {
    std::array<int, kNumValidCombos> a{};
    int n = 0;
    for (int i = 0; i < 64; ++i)
        if ((i / 8) != (i % 8)) a[n++] = i;
    return a;
