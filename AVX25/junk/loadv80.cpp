...
// PSEBuffer
//
// FIX 6: Added y_sum_acc[MAX_SIZE] and y_mean[MAX_SIZE].
...
//         scene_drift — the scene-wide brightness swing across the 8-frame
        ...
        uint64_t  valid_combo_mask)
{
    ...
    while (mask) {
        ...
        if (__builtin_expect((valid_combo_mask >> combo) & 1u, 1)) {
            ...
            // FIX 2: classify direction
            if (ms < xs) d2b_counters[combo]++;   // darkest then brightest → rising
            else         b2d_counters[combo]++;    // brightest then darkest → falling
        }
    }
}
...
// ============================================================================
// build_valid_combo_mask — FIX A (adjacency gate, unchanged from v59)
// ============================================================================
...
// ============================================================================
static inline double pts_to_elapsed(int64_t pts, const PSEBuffer& buf) noexcept {
...
// ============================================================================
void process_pse_temporal_avx_threaded(PSEBuffer&           buffer,
                                       ...
                                       PSEState&            state) noexcept
{
    ...
    auto process_y = [&] (int start, int end,
                          uint32_t* combos, uint32_t* d2b, uint32_t* b2d,
                          std::vector<uint32_t>& masks,
                          std::vector<ChunkVectors>& flashes) -> int
    {
        ...
        for (int chunk = start; chunk < end; ++chunk) {
            ...
            if (use_sdr) {
                ...
                uint32_t mlo = _mm256_movemask_epi8(_mm256_cmpgt_epi16(_mm256_mullo_epi16(dlo, _mm256_set1_epi16(17)), slo));
                uint32_t mhi = _mm256_movemask_epi8(_mm256_cmpgt_epi16(_mm256_mullo_epi16(dhi, _mm256_set1_epi16(17)), shi));
        ...
        // PASS 2 — FIX 2: directional attribution
        {
            int fi = 0;
            for (int chunk = start; chunk < end; ++chunk) {
                uint32_t mask = masks[chunk - start];
                if (!mask) continue;
                ChunkVectors& cv = flashes[fi++];
                extract_and_count_combos_precomputed(
                    mask, cv.frame_min_idx, cv.frame_max_idx,
    ...
    auto process_r = [&](uint32_t* combos, uint32_t* d2b, uint32_t* b2d,
                          ...
                          std::vector<ChunkVectors>& r_flashes) -> int
    {
        ...
        // PASS 2 — FIX 2: directional attribution on R
        {
            int fi = 0;
            for (int chunk = 0; chunk < n_chunks; ++chunk) {
                uint32_t mask = r_masks[chunk];
                if (!mask) continue;
                ChunkVectors& cv = r_flashes[fi++];
                extract_and_count_combos_precomputed(
                    mask, cv.frame_min_idx, cv.frame_max_idx,
                    combos, d2b, b2d, valid_mask);
            }
    ...
    {
        auto t1 = [&]() {
            flash_y_t1 = process_y(0, t1_y_end,
                                   t1_y_combos, t1_y_d2b, t1_y_b2d,
                                   state.mask_cache_t1, state.flash_cache_t1);
        };
        auto t2 = [&]() {
            flash_y_t2 = process_y(t1_y_end, buffer.y_chunks,
    ...
    for (int i = 0; i < 64; ++i) {
        if ((i/8) == (i%8)) continue;
        float cy = (float)tracker.y_combo_hits[i] / effective_y_pix_safe;
        float cr = (float)tracker.r_combo_hits[i] / (float)buffer.v_frame_size;
    ...
    for (int i = 0; i < 64; ++i) {
        if ((i/8) == (i%8)) continue;
        if (total_flash_y > 0) {
            if (tracker.y_d2b_hits[i] > 0.25f * total_flash_y) has_d2b_y = true;
            if (tracker.y_b2d_hits[i] > 0.25f * total_flash_y) has_b2d_y = true;
        }
        if (flash_r > 0) {
            if (tracker.r_d2b_hits[i] > 0.25f * flash_r) has_d2b_r = true;
            if (tracker.r_b2d_hits[i] > 0.25f * flash_r) has_b2d_r = true;
        }
    }
...
static constexpr int  ANALYSIS_W       = 480;   // default: 720p wide
static constexpr int  ANALYSIS_H       =  270;   // default: 720p tall
...
int main(int argc, char* argv[]) {
    ...
    // determine the lowest flash frequency this instance can reliably detect.
    {
        ...
        const double win_span_s       = (PSEBuffer::MAX_SIZE - 1) / fps;
